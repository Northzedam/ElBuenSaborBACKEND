...
public interface PedidoRepository extends JpaRepository<Pedido, Long> {
	...
	@Query("SELECT p FROM Pedido p "
			...
			+ "AND p.fecha > ?5 "
			+ "AND p.fecha < ?6 "
			+ "AND ((?7 IS TRUE AND p.horaFin IS NOT NULL) OR (?7 IS FALSE AND p.horaFin IS NULL)) "
			+ "AND ((?8 IS TRUE AND p.fechaAnulado IS NOT NULL) OR (?8 IS FALSE AND p.fechaAnulado IS NULL)) ")			
			
	Page<Pedido> findByNombre(boolean usarParamConEnvio, boolean conEnvio, long idEstadoPedido, String frase, Date fechaDesde, Date fechaHasta, boolean tieneHoraFin, boolean tieneFechaAnulado, Pageable pageable);
